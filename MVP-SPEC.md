...
- Triggers tie to existing surfaces, not new value: **bulk-import complete** (the
  ...
  *not* "credits expiring": monthly credits don't accrue, so nothing expires).
...
- **Submission generation** — a **monthly allotment** (reserve→commit/refund, §4) that renews on the calendar-month boundary.
- **Resume parsing (ingest)** — period-aware per tier: a **lifetime onboarding quota** on free (so the §2A backlog-dump never paywalls on day 1), a **monthly quota** on Solo Pro, and unmetered on Team.

- **Free (Solo) — $0:** full pool, **semantic search**, manual entry, jobs + trail + constraint filter, revenue view — all free forever. **20 submissions/month** + a generous **500-parse lifetime ingest quota**. Hitting a meter is the upgrade prompt — never a day-1 paywall.
- **Solo Pro — ~$29/mo:** single seat, **100 submissions/month** (effectively unlimited for a solo), **200 parses/month**, forwarding inbox. Captures the full-time solo biller the incumbents (~$99–169/seat) price out — monetization without a blocker.
...
**Credit math is safe** — a resume parse on Haiku 4.5 (~2,200 in / ~500 out) costs a fraction of a cent; a submission generation is similar. The free monthly submissions + the lifetime ingest quota ≈ pennies of COGS (a 500-CV backlog dump is ~$1–2 of Haiku). Credits exist to drive upgrade intent and cap abuse, **not** to cover cost.
